// Custom Flutter web bootstrap: force the CanvasKit renderer.
//
// Flutter 3.44 defaults to auto-selecting the "skwasm" renderer, which
// depends on SharedArrayBuffer and therefore requires cross-origin isolation
// (COOP: same-origin + COEP: require-corp response headers). The Electron
// desktop wrapper serves the bundle from a plain local HTTP server that does
// not (and, without breaking cross-origin Supabase calls, cannot easily) send
// those headers, so skwasm fails to create a rendering surface and the app
// shows a blank white screen. CanvasKit has no such requirement and renders
// reliably everywhere, so we pin the renderer to it explicitly.
...
_flutter.loader.load({
  ...
  serviceWorkerSettings: {
    serviceWorkerVersion: "3875046623" /* Flutter's service worker is deprecated and will be removed in a future Flutter release. */,
  },
});
